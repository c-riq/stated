...
function updateFixtures() {
    const fixtureDirs = fs.readdirSync(fixturesDir, { withFileTypes: true })
    ...
    for (const dir of fixtureDirs) {
        ...
        if (!fs.existsSync(inputPath)) {
            console.log(`Skipping ${dir}: no input.json found`);
            continue;
        }
        ...
        try {
            ...
            console.log(`✓ Updated ${dir}/output.txt`);
        } catch (error) {
            console.error(`✗ Error processing ${dir}:`, (error as Error).message);
        }
    }
}
