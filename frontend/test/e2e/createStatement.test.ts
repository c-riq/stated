...
test("create simple statement", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Create Statement" }),
  ).toBeVisible();
  await page
    .getByRole("textbox", { name: "Statement" })
    .fill("test basic statement");
  ...
  await page
    .getByRole("button", { name: "Publish using an API key for localhost" })
    .click();
  ...
  await expect(
    page.getByRole("heading", { name: "Statements (1)" }),
  ).toBeVisible();
});

test("url search query", async ({ page }) => {
  await page.goto(
    "http://localhost:3000/?search_query=test%20basic%20statement&types=Statements,Votes",
  );
