...
export const DecryptedContent = ({
  ...
  const [algorithmObject, setAlgorithmObject] = React.useState<string[]>(
    algorithm.toLowerCase() === "aes" ? ["AES", "AES"] : ["", ""],
  );
  ...
  try {
    if (algorithm.toLowerCase() === "aes") {
      const decrypted = AES.decrypt(
        fromUrlSafe(statement?.content).replace(/\n/g, ""),
        key,
      );
  ...
  return (
    <>
      ...
      <Autocomplete
        ...
        renderInput={(params) => (
          <TextField {...params} label="Decryption algorithm" />
        )}
        // @ts-ignore
      ...
      <TextareaAutosize
        style={{
          width: "100%",
          height: ("" + decryptedContent).match(/\n/g)
            ? 40 +
              (("" + decryptedContent).match(/\n/g)?.length || 0) * 18 +
              "px"
            : "60px",
          overflow: "scroll",
