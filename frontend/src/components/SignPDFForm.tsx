...
export const filePath = (hash: string, host: string | undefined) =>
  (host || backendHost) + "/files/" + hash + ".pdf";

export const getWorkingFileURL = async (hash: string, host: string) => {
  const promises = [host, backendHost].map((h) =>
    fetch(h + "/files/" + hash + ".pdf", { method: "OPTIONS" }),
  );
  ...
  return (
    <FormControl sx={{ width: "100%" }}>
      {fileHash ? (
        <embed
          src={
            fileURL ? fileURL : filePath(fileHash, originalHost || backendHost)
          }
          width="100%"
          height="300px"
