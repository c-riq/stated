...
export const FullVerificationGraph = () => {
  ...
  const [organisationVerifications, setOrganisationVerifications] =
    React.useState(
      [] as (StatementWithSupersedingDB & OrganisationVerificationDB)[],
    );
  ...
  React.useEffect(() => {
    if (!dataFetched) {
      getDomainVerifications(undefined, (statements) => {
        let verifications = statements as (StatementWithSupersedingDB &
          OrganisationVerificationDB)[];
  ...
  useEffect(() => {
    ...
    organisationVerifications.forEach(
      (
        ...
        const parsedOrganisationVerification =
          parseOrganisationVerification(content);
        if (!author) {
        ...
        if (!hash_b64) {
          hash_b64 = statement_hash || "hash_b64";
        }
        ...
        const sourceId = (verifier_domain + ":" + author)
          .replace(/ /g, "_")
          .toLowerCase();
        const targetId = ((verified_domain || foreign_domain) + ":" + name)
          .replace(/ /g, "_")
          .toLowerCase();
        ...
        if (!nodes.map((n) => n?.data?.id).includes(targetId)) {
          ...
          nodes.push({
            data: {
              ...
              color:
                legal_entity_type === legalForms.corporation
                  ? "rgba(42,74,103,1)"
                  : legal_entity_type === legalForms.local_government
                    ? "rgba(42,100,103,1)"
                    : legal_entity_type === legalForms.foreign_affairs_ministry
                      ? "rgba(200,42,42,1)"
                      : "rgba(42,42,42,1)",
              size:
                parsedOrganisationVerification.employeeCount ===
                peopleCountBuckets["100000"]
                  ? "50px"
                  : "30px",
            },
          });
        ...
        edges.push({
          data: {
            ...
            href: `${backendHost}/statements/${hash_b64}`,
            color:
              parsedOrganisationVerification &&
              (parsedOrganisationVerification.confidence ?? 0 >= 0.8)
                ? "rgb(150,150,150)"
                : "rgb(200,200,200)",
          },
        });
  ...
  return (
    <div
      ref={graphRef}
      style={{ width: "100vw", height: "100vh", textAlign: "start" }}
    ></div>
  );
};
