...
export const CompactStatement = (props: {
  ...
  setStatementToJoin: (s: StatementWithDetailsDB) => void;
  setModalOpen: () => void;
  i: string;
}) => {
  const { s, i } = props;
    ...
    >
      ...
      >
        ...
        >
          ...
          {s.content.split("\n").map((s1: string, i: number) => (
            <div key={i}>{highlightedStatement(s1, s.type || "")}</div>
          ))}
        </div>
      </Link>
    </div>
...
export const CompactStatementSmall = (props: {
  s: StatementDB;
      ...
      <div style={{ display: "flex", flexDirection: "row" }}>
        ...
        <div
          ...
          style={{
            ...
            flexGrow: 1,
            "a:textDecoration": "none",
            backgroundColor: "rgba(251,235,234,1)",
            padding: "18px",
            borderRadius: 8,
          }}
        >
          ...
          {s.content.split("\n").map((s1: string, i: number) => (
            <div key={i}>{highlightedStatement(s1, s.type || "", true)}</div>
          ))}
        </div>
      </div>
    </Link>
