...
export const VoteForm = (
  props: FormProps & { poll?: { statement: string; hash_b64: string } },
) => {
  const statementParsed = (props.poll?.statement &&
    parseStatement({
      statement: props.poll.statement,
      allowNoVersion: true,
    })) as Statement | undefined;
  const pollParsed = (statementParsed && parsePoll(statementParsed.content)) as
    | Poll
    | undefined;

  const [pollHash, setPollId] = React.useState(props.poll?.hash_b64 || "");
  const [poll, setPoll] = React.useState(pollParsed?.poll || "");
  const [pollStatement, setPollStatement] = React.useState(
    undefined as StatementDB | undefined,
  );
  const [options, setOptions] = React.useState(
    (pollParsed?.options || []) as string[],
  );
  ...
  const prepareStatement: prepareStatement = ({ method }) => {
    try {
      ...
      let voteString =
        vote === "other" || !options.length ? freeTextVote : vote;
      const content = buildVoteContent({
        pollHash: pollHash,
        poll: poll,
        vote: voteString,
      });
  ...
  return (
    <FormControl sx={{ width: "100%" }}>
      ...
      {pollStatement && (pollStatement as StatementDB)?.content ? (
        <a
          style={{ color: "#1976d2" }}
          href={`/statements/${(pollStatement as StatementDB).hash_b64}`}
          target="_blank"
          rel="noreferrer"
        >
      ...
      <FormLabel id="polllabel" style={{ marginTop: "12px" }}>
        {poll}
      </FormLabel>
      {options.length > 0 && (
        <RadioGroup value={vote} onChange={handleChange}>
          {options.map((o, i) => (
            <FormControlLabel
              key={i}
              value={o}
              control={<Radio data-testid={"option-" + (i + 1)} />}
              label={o}
            />
          ))}
          {allowArbitraryVote && (
      ...
      {allowArbitraryVote && (!options.length || vote === "other") && (
        <TextField
          ...
          onChange={(e) => {
            setFreeTextVote(e.target.value);
          }}
          margin="normal"
          sx={{ marginBottom: "12px" }}
        />
