...
const CreateStatement = (props: Props) => {
  const [content, setContent] = React.useState(
    props.statementToJoin?.content || "",
  );
  const [_type, setType] = React.useState(
    props.poll
      ? statementTypes.vote
      : props.statementToRespond
        ? statementTypes.response
        : props.statementToDisputeAuthenticity
          ? statementTypes.disputeAuthenticity
          : props.statementToDisputeContent
  ...
  const [statedVerification, setStatedVerification] = React.useState(
    [] as (StatementDB & OrganisationVerificationDB)[],
  );
  ...
  const [supersededStatement, setSupersededStatement] = React.useState(
    props.statementToSupersede?.hash_b64 ?? "",
  );
  const [showAdditionalFields, setShowAdditionalFields] = React.useState(false);
  const [apiKey, setApiKey] = React.useState("");
  const [publishingMethod, setPublishingMethod] = React.useState(
    undefined as publishingMethod | undefined,
  );
  const [dnsResponse, setDnsResponse] = React.useState([] as string[]);
  const [staticResponse, setStaticResponse] = React.useState(
    undefined as { validated: boolean; response?: string } | undefined,
  );
  ...
  React.useEffect(() => {
    getDomainSuggestions(domainInputValue, (res) => {
      ...
      const domains = res.result.map((r) => ({
        ...r,
        domain: r.domain.replace(/^stated\./, "").replace(/^www\./, ""),
      }));
      const uniqueDomains = [
        ...new Set(
          res.result.map((r) =>
            r.domain.replace(/^stated\./, "").replace(/^www\./, ""),
          ),
        ),
      ].map((d) => domains.find((r) => r.domain === d));
  ...
  React.useEffect(() => {
    ...
    getSSLOVInfo(domain, (res) => {
      const OVInfo = res
        ? (res.result || [])
            .filter((r: PromiseSettledResult<ssl>) => r.status === "fulfilled")
            .map((r: PromiseFulfilledResult<ssl>) => r.value)
        : [];
      const matchingOV = OVInfo.find(
        (r: ssl) =>
          (r.domain === domain ||
  ...
  const checkStaticStatementAPI = () => {
    checkStaticStatement(
      { hash: statementHash, statement: statement, domain },
      (res) => {
  ...
  const submitStatementAPI = () => {
    ...
    submitStatement(
      ...
      (error) => {
        setAlertMessage(
          "Error: could not submit statement! " + JSON.stringify(error),
        );
        setisError(true);
      },
    );
    ...
    <>
      ...
      <Autocomplete
        ...
        isOptionEqualToValue={(option, value) =>
          option && value && !!(option.domain && option.domain === value.domain)
        }
        ...
        onChange={(event, newInputValue: string | domainOption) => {
          ...
          !author &&
            setAuthor(
              newInputValue?.department
                ? newInputValue.department
                : newInputValue?.organisation,
            );
        }}
        ...
        renderInput={(params) => (
          <TextField
            {...params}
            label="Your domain used for publishing/ authenticating"
      ...
      />
      {OVInfo &&
        OVInfo.reduce((acc, i) => acc || i.domain === domain, false) &&
        (OVInfo.reduce((acc, i) => acc || i.O, "") ? (
          OVInfo.filter((i) => i.O).map((i, k) => (
            <Alert key={k} severity="success" style={{ marginTop: "10px" }}>
              Verified via{" "}
              <a target="_blank" href={"https://crt.sh/?sha256=" + i.sha256}>
                {" "}
                SSL certificate {i.domain + ": " + i.O + " by " + i.issuer_o}
              </a>
            </Alert>
          ))
        ) : (
          <Alert severity="warning" style={{ marginTop: "10px" }}>
            Organisation not verified via SSL certificate.
          </Alert>
        ))}
      {statedVerification &&
        statedVerification.reduce(
          (acc, i) => acc || i.verified_domain === domain,
          false,
        ) &&
        (statedVerification.reduce(
          (acc, i) => acc || (i.verified_domain ?? ""),
          "",
        ) ? (
          [
            statedVerification.find(
              (i) => i.verified_domain === domain && i.name,
            ),
          ].map((i, k) => (
            <Alert key={k} severity="success" style={{ marginTop: "10px" }}>
              Verified via stated verification{" "}
              <a
                target="_blank"
                href={
                  window.location.origin + "/statements/" + i!.statement_hash
                }
              >
                {i!.verified_domain +
                  ": " +
                  (i!.department ? i!.department + " at " : "") +
                  i!.name +
                  " by " +
                  i!.verifier_domain}
              </a>
          ...
          <Alert severity="warning" style={{ marginTop: "10px" }}>
            Not verified via stated verification.
          </Alert>
        ))}
      {DNSSECInfo &&
        DNSSECInfo.domain === domain &&
        (DNSSECInfo.validated ? (
          <Alert severity="success" style={{ marginTop: "10px" }}>
            DNSSEC enabled for {DNSSECInfo.domain}
          </Alert>
        ) : (
          <Alert severity="warning" style={{ marginTop: "10px" }}>
            DNSSEC not enabled for {DNSSECInfo.domain}
          </Alert>
        ))}
      ...
      {props.statementToSupersede && (
        <TextField
          ...
          onChange={(e) => {
            setSupersededStatement(e.target.value);
          }}
          margin="normal"
          style={{ backgroundColor: "#eeeeee", marginTop: "16px" }}
        />
    ...
    >
      <div>
        <h3 style={{ marginBottom: "50px" }}>
          {props.statementToJoin ? "Join Statement" : "Create Statement"}
        </h3>
        ...
        <FormControl style={{ width: "50vw", maxWidth: "500px" }}>
          ...
          <Select
            ...
            label="Type"
            onChange={(e) =>
              handleTypeChange(e.target.value as StatementTypeValue)
            }
          ...
          >
            ...
            <MenuItem
              value={statementTypes.organisationVerification}
              data-testid="organisation-verification"
            >
              Verify an organisation
            </MenuItem>
            <MenuItem value={statementTypes.personVerification}>
              Verify a person
            </MenuItem>
            <MenuItem value={statementTypes.rating}>Rating</MenuItem>
            <MenuItem value={statementTypes.poll} data-testid="poll">
              Poll
            </MenuItem>
            <MenuItem value={statementTypes.vote} data-testid="vote">
              Vote
            </MenuItem>
            <MenuItem value={statementTypes.disputeAuthenticity}>
              Dispute statement authenticity
            </MenuItem>
            <MenuItem value={statementTypes.disputeContent}>
              Dispute statement content
            </MenuItem>
          ...
          {_type === statementTypes.organisationVerification && (
            <OrganisationVerificationForm
              ...
              setStatementHash={setStatementHash}
              serverTime={props.serverTime}
              setisError={setisError}
              setAlertMessage={setAlertMessage}
              setPublishingMethod={setPublishingMethod}
            >
              {authorFields()}
            </OrganisationVerificationForm>
          )}
          {_type === statementTypes.personVerification && (
          ...
          {_type === statementTypes.disputeAuthenticity && (
            <DisputeStatementAuthenticityForm
              metaData={{
                domain,
                author,
                representative,
              ...
              setAlertMessage={setAlertMessage}
              setPublishingMethod={setPublishingMethod}
              statementToDisputeAuthenticity={
                props.statementToDisputeAuthenticity
              }
            >
              {authorFields()}
          ...
          {_type === statementTypes.disputeContent && (
            <DisputeStatementContentForm
              metaData={{
                domain,
                author,
                representative,
          ...
          {_type === statementTypes.statement && (
            <StatementForm
              metaData={{
                domain,
                author,
              ...
              statementToJoin={props.statementToJoin}
              setStatement={setStatement}
              setStatementHash={setStatementHash}
              serverTime={props.serverTime}
              setisError={setisError}
          ...
          {_type === statementTypes.observation && (
            <ObservationForm
              metaData={{
                domain,
                author,
          ...
          {publishingMethod === "dns" && statementHash.length > 0 && (
            <div style={{ paddingTop: "20px", width: "100%" }}>
              <span>
                Add the following TXT record in your {domain} domain settings to
            ...
            <div style={{ paddingTop: "20px", width: "100%" }}>
              {dnsResponse.includes(statementHash) ? (
                ...
                <div>
                  ...
                  >
                    Error: TXT records should include {statementHash}
                  </Alert>
                  <div
                    style={{
                      fontSize: "8pt",
          ...
          {publishingMethod === "static" && statementHash.length > 0 && (
            <div style={{ paddingTop: "20px", width: "100%" }}>
              <span>
                ...
                Publish this{" "}
                <a
                  download={statementHash + ".txt"}
                  href={
                    "data:application/octet-stream;charset=utf-8;base64," +
                    window.btoa(statement)
                  }
                >
                ...
                >
                  {`https://static.stated.${domain}/statements/${statementHash}.txt`}
                </Link>
                ...
                >
                  {`https://www.${domain}/.well-known/statements/${statementHash}.txt`}
                </Link>
                <span>&nbsp;</span>to verify domain ownership.
              </span>
              <p>
                You can follow these instructions to publish the file and link
                it with your 'static.stated.' subdomain:
              </p>
              <ul>
                <li>
                  <Link href="https://github.com/c-riq/stated/blob/master/static/netlify/README.md">
                    Instructions for hosting on Netlify
                  </Link>
                </li>
                <li>
                  <Link href="https://github.com/c-riq/stated/blob/master/static/github-pages/README.md">
            ...
            <div style={{ paddingTop: "20px", width: "100%" }}>
              {staticResponse.validated ? (
                ...
                <div>
                  ...
                  >
                    Could not retrieve statement. Response:{" "}
      ...
      <Portal>
        <Snackbar
          open={alertMessage !== undefined && alertMessage.length > 0}
