...
const DisputeStatementAuthenticityForm = (
  props: FormProps & {
    statementToDisputeAuthenticity?: StatementDB | StatementWithDetailsDB;
  },
) => {
  const [disputedStatementHash, setDisputedStatementHash] = React.useState(
    props.statementToDisputeAuthenticity?.hash_b64 || "",
  );
  const [confidence, setConfidence] = React.useState("");
  const [reliabilityPolicy, setReliabilityPolicy] = React.useState("");

  const [referencedStatement, setReferencedStatement] = React.useState(
  ...
  React.useEffect(() => {
    ...
    getStatement(hashQuery, (res) => {
      if (hashQuery !== disputedStatementHash) {
        return;
      }
  ...
  const prepareStatement: prepareStatement = ({ method }) => {
    try {
      ...
      const content = buildDisputeAuthenticityContent({
        hash: disputedStatementHash,
  ...
  return (
    <FormControl sx={{ width: "100%" }}>
      <TextField
        ...
        onChange={(e) => {
          setDisputedStatementHash(e.target.value);
        }}
        margin="normal"
      ...
      {referencedStatement && (referencedStatement as StatementDB)?.content ? (
        <a
          style={{ color: "#1976d2" }}
          href={`/statements/${(referencedStatement as StatementDB).hash_b64}`}
          target="_blank"
        >
