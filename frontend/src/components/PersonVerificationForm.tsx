...
const PersonVerificationForm = (props: FormProps) => {
  ...
  const prepareStatement: prepareStatement = ({ method }) => {
    try {
      ...
      const content = buildPersonVerificationContent({
        name: verifyName,
        ...(ownsDomain ? { ownDomain: verifyDomain } : { foreignDomain }),
    ...
    <FormControl sx={{ width: "100%" }}>
      ...
      {ownsDomain ? (
        ...
        <TextField
          ...
          onChange={(e) => {
            setForeignDomain(e.target.value);
          }}
          margin="normal"
          sx={{ marginBottom: "24px" }}
        />
      ...
      <Autocomplete
        ...
        renderInput={(params) => (
          <TextField {...params} label="Country of birth" />
        )}
        sx={{ marginTop: "20px" }}
      />
      <Autocomplete
