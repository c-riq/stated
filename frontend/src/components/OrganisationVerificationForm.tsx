...
const OrganisationVerificationForm = (props: FormProps) => {
  ...
  const [provinceObject, setProvinceObject] = React.useState(
    undefined as string[] | undefined,
  );
  ...
  const [employeeCountObject, setEmployeeCountObject] = React.useState(
    undefined as string | undefined,
  );
  ...
  const prepareStatement: prepareStatement = ({ method }) => {
    ...
    try {
      const content = buildOrganisationVerificationContent({
        name: verifyName,
        domain: verifyDomain,
        city,
        country,
  ...
  return !(provinces.length && cities.length && countries.length) ? (
    ...
    <FormControl sx={{ width: "100%" }}>
      <TextField
        ...
        onChange={(e) => {
          setVerifyDomain(e.target.value);
        }}
        margin="normal"
      ...
      <Autocomplete
        ...
        renderInput={(params) => (
          <TextField {...params} label="Headquarter country" required />
        )}
        sx={{ marginTop: "20px" }}
      />
      <Autocomplete
        ...
        renderInput={(params) => (
          <TextField {...params} label="Legal form" required />
        )}
        // @ts-ignore
        renderOption={(props, option) => (
          <Box {...props} id={option}>
      ...
      <Autocomplete
        ...
        data-testid="city"
        options={
          countryObject ? cities.filter((l) => l[2] === countryObject[4]) : []
        }
        autoHighlight
        ...
        renderInput={(params) => (
          <TextField {...params} label="Headquarter city" />
        )}
        // @ts-ignore
        renderOption={(props, option) => (
      ...
      <Autocomplete
        ...
        options={
          countryObject
            ? provinces.filter((l) => l[0] === countryObject[1])
            : []
        }
        autoHighlight
        ...
        renderInput={(params) => (
          <TextField {...params} label="Province / state" />
        )}
        // @ts-ignore
        renderOption={(props, option) => (
          <Box {...props} id={option[0] + "_" + option[1]}>
            {option[2]}
          </Box>
        )}
        sx={{ marginTop: "20px" }}
      ...
      <Autocomplete
        ...
        onInputChange={(event, newInputValue) =>
          setEmployeeCount(newInputValue)
        }
        renderInput={(params) => (
      ...
      <TextField
        ...
        label={
          "Business register serial number" +
          (countryObject?.[1] === "US"
