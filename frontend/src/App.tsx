...
const typesFromUrl = urlParams
  .get("types")
  ?.split(",")
  .map((t: string) =>
    backwardsCompatibility[t] ? backwardsCompatibility[t] : t,
  )
...
function App() {
  ...
  const [statementToJoin, setStatementToJoin] = React.useState(
    undefined as (StatementWithDetailsDB | StatementDB) | undefined,
  );
  const [statementToRespond, setStatementToRepsond] = React.useState(
    undefined as (StatementWithDetailsDB | StatementDB) | undefined,
  );
  const [statementToDisputeAuthenticity, setStatementToDisputeAuthenticity] =
    React.useState(
      undefined as (StatementWithDetailsDB | StatementDB) | undefined,
    );
  const [statementToDisputeContent, setStatementToDisputeContent] =
    React.useState(
      undefined as (StatementWithDetailsDB | StatementDB) | undefined,
    );
  const [statementToSupersede, setStatementToSupersede] = React.useState(
    undefined as (StatementWithDetailsDB | StatementDB) | undefined,
  );
  const [poll, setPoll] = React.useState(
    undefined as { statement: string; hash_b64: string } | undefined,
  );
  const [statements, setStatements] = React.useState(
    [] as StatementWithDetailsDB[],
  );
  const [modalOpen, setModalOpen] = React.useState(false);
  const [postToView, setPostToView] = React.useState(false);
  const [searchQuery, setSearchQuery] = React.useState(
    queryFromUrl || (undefined as string | undefined),
  );
  const [lt850px, setlt850px] = React.useState(
    window.matchMedia("(max-width: 850px)").matches,
  );
  const [lt500px, setlt500px] = React.useState(
  ...
  const [statementTypesFilter, setStatementTypesFilter] = React.useState<
    string[]
  >(typesFromUrl || []);
  ...
  const [domainFilter, setDomainFilter] = React.useState<string | undefined>(
    domainFilterFromUrl || undefined,
  );
  const [authorFilter, setAuthorFilter] = React.useState<string | undefined>(
    auhtorFilterFromUrl || undefined,
  );
  const [tagFilter, setTagFilter] = React.useState<string | undefined>(
    tagFilterFromUrl || undefined,
  );
  const [triggerUrlRefresh, setTriggerUrlRefresh] =
  ...
  React.useEffect(() => {
    window
      ...
      .addEventListener("change", (e) => setlt850px(e.matches));
    window
      .matchMedia("(max-width: 500px)")
      .addEventListener("change", (e) => setlt500px(e.matches));
  }, []);
  ...
  useEffect(() => {
    updateQueryString({
      ...
      domainFilter,
      authorFilter,
      statementTypes: statementTypesFilter,
    });
  }, [
    searchQuery,
    tagFilter,
    statementTypesFilter,
    domainFilter,
    authorFilter,
    triggerUrlRefresh,
  ]);
  ...
  React.useEffect(() => {
    if (
      location.pathname.match("full-verification-graph") ||
      location.pathname.match("full-network-graph")
    ) {
      return;
    }
    const limit = 20;
    getStatements({
      ...
      author: authorFilter,
      cb: (
        s: { statements: StatementWithDetailsDB[]; time: string } | undefined,
      ) => {
        if (s?.statements && s.statements.length > 0) {
          setStatements(s.statements);
          const globalMaxSkipId = parseInt(s.statements[0].max_skip_id);
          const currentMaxSkipId = s.statements.reduce(
            (max: number, s: StatementWithDetailsDB) =>
              Math.max(max, parseInt(s.skip_id)),
            0,
          );
      ...
      getStatements({
        ...
        cb: (
          ...
          if (s?.statements && s.statements.length > 0) {
            ...
            const newStatements = s.statements.filter(
              (s: StatementWithDetailsDB) =>
                !existingStatements.find(
                  (s2: StatementWithDetailsDB) => s2.hash_b64 === s.hash_b64,
                ),
            );
  ...
  const supersedeStatement = ({
    ...
    if (pollOfVote) {
      setPoll({
        statement: pollOfVote?.statement as string,
        hash_b64: pollOfVote?.hash_b64 as string,
      });
    }
  ...
  const resetState = () => {
    navigate("/");
    setModalOpen(false);
    setStatementToJoin(undefined);
    setPostToView(false);
    setStatementToRepsond(undefined);
    setStatementToDisputeAuthenticity(undefined);
    setStatementToDisputeContent(undefined);
    setStatementToSupersede(undefined);
    setPoll(undefined);
    ...
    <div className="App" style={{ overflow: modalOpen ? "hidden" : "scroll" }}>
      ...
      <div className="App-main">
        <Routes>
          <Route
            element={
              <Layout
                ...
                setStatementTypes={setStatementTypesFilter}
                maxSkipId={maxSkipId}
            ...
            <Route
              ...
              element={
                <CenterModal
                  modalOpen={true}
                  lt850px={lt850px}
                  onClose={({ warning }: { warning: string }) => {
                    warning ? setDialogOpen(true) : resetState();
                  }}
                >
                  <CreateStatement
                    serverTime={serverTime}
                    statementToJoin={statementToJoin}
                    statementToRespond={statementToRespond}
                    statementToDisputeAuthenticity={
                      statementToDisputeAuthenticity
                    }
                    statementToDisputeContent={statementToDisputeContent}
          ...
          <Route
            path="/full-verification-graph"
            element={<FullVerificationGraph />}
          />
